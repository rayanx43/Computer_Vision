...
def compute_distances(X1, X2):
    ...
    """
    Computing standard euclidean distances for X1 and X2 features
    """
    X1_square = np.sum(np.square(X1), axis=1)
    X2_square = np.sum(np.square(X2), axis=1)
    # Computing the dists array with proper normalized distances
...
def predict_labels(dists, y_train, k=1):
    ...
    for i in range(num_test):
        ...
        #getting the indexes of the closest neighbours
        idx = np.argsort(dists[i, :])
        #storing the labels in closest_y using y_train
        for j in range(k):
            closest_y.append(y_train[idx[j]])
        #storing the most common label in closest_y in y_pred[i]
...
def split_folds(X_train, y_train, num_folds):
    ...
    #Using np.array_split function get the folds from the array
    X_lst = np.array_split(X_train, num_folds, axis=0)
    #reshaping y_train for further manipulation
    y_train = y_train.reshape(-1, 1)
    #Getting folds from y_train too
    y_lst = np.array_split(y_train, num_folds, axis=0)
    #Computing the final folds using a for loop
