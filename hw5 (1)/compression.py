...
def compress_image(image, num_values):
    ...
    """
    Computation of SVD using np.linalg.svd
    """
    u, s, v = np.linalg.svd(image)
    """
    Computing the compressed image using ONLY the top singular values of
    each matrix u, s and v.
    """
    compressed_image = np.dot(u[:, :num_values].dot(np.diag(s[:num_values])), v[:num_values, :])
    """
    Computing the compressed size accordingly
    """
