...
class PCA(object):
    ...
    def _svd(self, X):
        ...
        #computing SVD using the np.linalg.svd function
    ...
    def transform(self, X, n_components):
        ...
        # Substracting mean stored during 'fit'
        X = X - self.mean
        # projecting into a subspace of dimension `n_components` using the dot product
    ...
    def reconstruct(self, X_proj):
        ...
        #projecting back onto the original space of dimension D 
        X = X_proj.dot(self.W_pca[:n_components, :])
        #adding the mean that we substracted in `transform`
        X = X + self.mean
...
class LDA(object):
    ...
    def fit(self, X, y):
        ...
        #using scipy.linalg.eig to compute eigenvalus and eigenvectors
    ...
    def _within_class_scatter(self, X, y):
        ...
        for i in np.unique(y):
            ...
            #Getting covariance matrix for class i 
            X_i = X[y == i]
            X_centered = X_i - np.mean(X_i, axis=0)
            S_i = X_centered.T.dot(X_centered)
            #adding it to scatter_within
            scatter_within += S_i
